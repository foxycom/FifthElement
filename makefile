...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=fifthelement.theelement  -Dabc.instrument.multithreaded "

INSTRUMENTATION_OPTS=" \
--skip-class=fifthelement.theelement.R \
--filter-package=java.io \
--filter-package=java.sql \
--filter-class=java.lang.StringBuilder \
--filter-class=java.lang.Object \
"
...
SELECT_ACTIVITIES_ALL_CARVING_OPTIONS=" \
$(CARVING_OPTIONS) \
--selection-strategy=SELECT_ACTIVITIES_ALL \
"

SELECT_ACTIVITIES_ONE_CARVING_OPTIONS=" \
$(CARVING_OPTIONS) \
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "fifthelement.theelement$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "fifthelement.theelement.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall fifthelement.theelement > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall fifthelement.theelement.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear fifthelement.theelement > /dev/null; \
fi)
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) fifthelement.theelement.test/android.support.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces fifthelement.theelement ./traces/$(TEST_NAME) force-clean
